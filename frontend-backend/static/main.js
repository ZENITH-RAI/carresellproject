(function () {
  ...
  function validate_form() {
    ...
    if (!the_form.brand.value.trim()) {
      mark_error("brand", "Enter a brand");
      ok = false;
    }
    if (!the_form.model.value.trim()) {
      mark_error("model", "Enter a model");
      ok = false;
    }
    var year = parseInt(the_form.year.value, 10);
    if (isNaN(year) || year < 1980 || year > 2030) {
      mark_error("year", "Use a year between 1980 and 2030");
      ok = false;
    }
    if (
      isNaN(parseFloat(the_form.km_driven.value)) ||
      +the_form.km_driven.value < 0
    ) {
      mark_error("km_driven", "Enter kilometers driven");
      ok = false;
    }
    if (!the_form.fuel.value) {
      mark_error("fuel", "Select a fuel type");
      ok = false;
    }
    if (!the_form.transmission.value) {
      mark_error("transmission", "Select a transmission type");
      ok = false;
    }
    if (!the_form.seller_type.value) {
      mark_error("seller_type", "Select a seller type");
      ok = false;
    }
    if (!the_form.owner.value) {
      mark_error("owner", "Select an owner type");
      ok = false;
    }
    if (
      isNaN(parseFloat(the_form.mileage.value)) ||
      +the_form.mileage.value <= 0
    ) {
      mark_error("mileage", "Enter valid mileage (kmpl)");
      ok = false;
    }
    if (
      isNaN(parseFloat(the_form.engine.value)) ||
      +the_form.engine.value <= 0
    ) {
      mark_error("engine", "Enter engine size (CC)");
      ok = false;
    }
    if (
      isNaN(parseFloat(the_form.max_power.value)) ||
      +the_form.max_power.value <= 0
    ) {
      mark_error("max_power", "Enter max power (bhp)");
      ok = false;
    }
    var seats = parseInt(the_form.seats.value, 10);
    if (isNaN(seats) || seats < 2 || seats > 12) {
      mark_error("seats", "Enter seats (2–12)");
      ok = false;
    }
  ...
  the_form.addEventListener("submit", function (event) {
    ...
    if (!validate_form()) {
      show_toast("Please fill up all the necessary fields.");
      return;
    }
